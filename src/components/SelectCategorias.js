...
const SelectCategorias = ({ inputs, setInputs }) => {
  const [centinelSelect, setCentinelSelect] = useState(false);
  const categorias = [
  ...
  const handleClick = (e) => {
    setInputs({ ...inputs, categoria: e.currentTarget.dataset.valor });
  };

  return (
    <ContenedorSelect onClick={() => setCentinelSelect(!centinelSelect)}>
      <OpcionSeleccionada>
        {inputs.categoria} <IconoDown />
      </OpcionSeleccionada>
      {centinelSelect && (
        <Opciones>
